...
def flag_broken_refs(cells: list[dict], G: nx.DiGraph) -> list[dict]:
    ...
    not present in the whitelist (i.e. not a cell we know exists in the model).
    build_graph adds phantom nodes for all referenced coords, so we compare
    against the explicit cells list rather than G.nodes().
    """
...
def auto_flag(cells: list[dict], G: nx.DiGraph) -> list[dict]:
    ...
    Logs a summary of issue counts per type.
    """
    issues: list[dict] = []
