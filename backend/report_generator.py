...
def build_json_report(
    ...
    cells_affected = len({
        (i.get("sheet"), i.get("cell"))
        for i in all_issues
    })

    # Gather graph analysis flags by issue_type
...
def _issue_row(issue: dict) -> str:
    ...
    if count > 1:
        ...
        inst_html = f'<div style="color:#64748b">1 cell</div>'
...
def build_html_report(report: dict) -> str:
    ...
    # Graph analysis section (only if there are flags)
    graph_content_parts = []
    ...
    sheet_options = "\n".join(f'<option value="{s}">{s}</option>' for s in sheets)
