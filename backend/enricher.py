...
Run only on Tier 1 + Tier 2 representatives. Tier 3 cells are not enriched.
"""
...
def _get_val(ws, col_letter: str | None, row: int):
    """Read a single cell value from the worksheet given a column letter and row."""
    if not col_letter:
        return None
    try:
...
def _get_val_by_rowcol(ws, row: int, col: int):
    """Read a single cell value from the worksheet given row and column integers."""
    try:
...
def enrich_cell(cell: dict, structure_map: dict, wb_values) -> dict:
    ...
    # Label: look up label column on the same row
    ...
    # Units: look up unit column on the same row
    unit_col = sm.get("unit_col")
    cell["units"] = _get_val(ws, unit_col, row) if (ws and unit_col) else None

    # Period: look up timeline row at the cell's column
    ...
    # Section: nearest section header at or above this row
    section_headers = sm.get("section_headers", [])
    cell["section"] = _find_section(section_headers, row)

    # Dependency details: label + computed value for each raw dependency
    dep_details = []
